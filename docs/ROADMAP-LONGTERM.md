...
> 这四门及之后先列到"批次 + 核心定理清单"，具体条目 id 在每批开工时按 §3.11 在文件头展开成与上表相同的表格。为什么不提前全部展开：这些批次量极大（R 有 5 批、后续每门数批），
...
1. 每批开工：`.lean` 文件头写"本文件当前条目"清单（§3.11），每条目一个 Entry（docstring 带 `> **Entry**: <id>`）。
